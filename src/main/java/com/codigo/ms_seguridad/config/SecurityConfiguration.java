...
public class SecurityConfiguration {
        ...
        httpSecurity.cors(cors-> cors.configure(httpSecurity))
                ...
                .authorizeHttpRequests(request ->
                        request.requestMatchers(Constants.ENDPOINTS_PERMIT).permitAll()
                                .requestMatchers(Constants.ENDPOINTS_USER).hasAnyAuthority(Role.CLIENTE.name())
                                .requestMatchers(Constants.ENDPOINTS_ADMIN).hasAnyAuthority(Role.SUPER_ADMIN.name())
                                .requestMatchers(Constants.ENDPOINTS_GESTOR_RESTAURANTE).hasAnyAuthority(Role.GESTOR_RESTAURANTE.name())
    ...
    public WebSecurityCustomizer webSecurityCustomizer() {
        return (web) -> web.ignoring()
                // 🟢 Esto le quita el control a Spring Security por completo para /uploads/ y todo lo que esté adentro
